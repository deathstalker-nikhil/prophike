...
public function get($id = '',$limit = 10,$fields,$where = '' ,$orderBy = 'id DESC')
	{
		...
		if ($id != '')
		{
			...
			$query = $this->db->get('builders',$limit);
		}
		else
		{
			$this->db->select($fields);
	...
	{
		return $this->db->delete('builders', array('id' => $id));
	}
	...
	*/
	...
	public function rowsCount()
	{
		$first_id = 0;
		$last_id = 0;
		$query = $this->db->query('select count(*) as total from builders');
		$query2 = $this->db->get('builders', 1);
		$this->db->order_by('id DESC');
		$query3 = $this->db->get('builders',1);
		if($query2->result()){
